...
- The Amazon QuickSight service role gets an additive inline policy named `<STACK_PREFIX>-QuickSightS3Access` (namespaced per deployment, since all deployments in an account share one QuickSight service role). It does not modify the console-managed `AWSQuickSightS3Policy`, so the Amazon QuickSight console retains ownership of that policy.
